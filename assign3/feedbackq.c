...
// sets the new quantum time
void new_time(Task_t *task) {
    if(task->current_queue == 3) {
        remaining_quantum = 8;
    } else if(task->current_queue == 2) {
        remaining_quantum = 4;
    } else {
        remaining_quantum = 2;
    }    
...
void handle_instruction(Instruction_t *instruction, int tick) {
	...
	if(instruction->burst_time == 0) { 
        ...
        // loads the wait and exe times then prints them
        ...
        // enqueues the task then sets the new burst times
...
Task_t *peek_priority_task() {
    ...
    if(!is_empty(queue_1)) {
        ...
        this_task = queue_2->start;
    } else if(!is_empty(queue_3)){
        this_task = queue_3->start;
    }
...
void scheduler() {
    ...
    // decreases current task level once quantum hits 0 and requeues it
    ...
    if(priority_task != NULL) {  
        ...
        if(current_task != NULL && current_task->current_queue <= priority_task->current_queue) {
            return;
...
void execute_task(int tick) {
	if(current_task != NULL) {
		// TODO ... Insert your Code Here
        // decrements remaining burst time, then increments total execution time and quantum
...
/*
 * Function: mainid != current_task->id
